...
def load_central_config(project_root: Path) -> dict[str, Any]:
    lch_dir = project_root / "_lch"
    return merge_layers(
        (
            load_toml(lch_dir / "config.toml", required=True),
            load_toml(lch_dir / "config.user.toml"),
            load_toml(lch_dir / "custom" / "config.toml"),
            load_toml(lch_dir / "custom" / "config.user.toml"),
        )
    )
...
def load_customization(project_root: Path | None, skill_dir: Path) -> dict[str, Any]:
    ...
    custom_dir = project_root / "_lch" / "custom" if project_root else None
    return merge_layers(
        (
