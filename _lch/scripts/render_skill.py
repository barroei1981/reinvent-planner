...
_SNAPSHOT_TOKEN = re.compile(r"\[\[lch-snapshot:([A-Za-z0-9_./-]+\.md)\]\]")
...
def render(project_root: Path, skill_dir: Path) -> Path:
    ...
    if not (project_root / "_lch").is_dir():
        raise RenderError(f"project root does not contain _lch/: {project_root}")
